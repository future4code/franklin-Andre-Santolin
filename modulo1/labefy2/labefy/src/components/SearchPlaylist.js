    ...
    render() {
        ...
        return (
            <All>
                ...
                {this.state.allPlaylists.map((list) => {
                    return (
                        <Container>
                            ...
                            <ContainerSub>
                                <H5Styled onClick={() => { if (window.confirm(`Tem certeza de que deseja deletar ${list.name}?`)) this.deletPlaylist(list.id) }}>Deletar</H5Styled>
                                <H5Styled onClick={this.onClickInfo}>info +</H5Styled>
                                <H5Styled onClick={this.onClickAddMusic}>Adicionar Música</H5Styled> 
                            </ContainerSub>
                            {pageInfo(list.id)}
                        </Container>
                    )
                ...
                <ContainerVolt>
                    <VoltarStyled onMouseOver={this.MouseOver} onMouseOut={this.MouseOut} onClick={this.props.onClickCreatePage}>Voltar</VoltarStyled>
                </ContainerVolt>
            </All>
        )
    }
